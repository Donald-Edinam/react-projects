import React from 'react'
import { Link } from 'react-router-dom'

const Navbar = () => {
    return (
        <>

            <header className="w-full h-20 mt-5 border-b-2 border-gray-300">
                <div className="nav flex items-center justify-around">
                    <div className="search-tab">
                        <div className="search-btn">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
                                <path strokeLinecap="round" strokeLinejoin="round" d="m21 21-5.197-5.197m0 0A7.5 7.5 0 1 0 5.196 5.196a7.5 7.5 0 0 0 10.607 10.607Z" />
                            </svg>

                        </div>
                    </div>
                    <div className="header_logo">
                        <h1 className="text-4xl">
                            NY TIMES
                        </h1>
                    </div>
                    <div className="header_utility flex gap-10">
                        <button className='flex items-center justify-between'>
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-6">
                                <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 6a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0ZM4.501 20.118a7.5 7.5 0 0 1 14.998 0A17.933 17.933 0 0 1 12 21.75c-2.676 0-5.216-.584-7.499-1.632Z" />
                            </svg>
                            <p>Sign In</p>
                        </button>
                        <button className='flex items-center justify-between'>
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-6">
                                <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 3h1.386c.51 0 .955.343 1.087.835l.383 1.437M7.5 14.25a3 3 0 0 0-3 3h15.75m-12.75-3h11.218c1.121-2.3 2.1-4.684 2.924-7.138a60.114 60.114 0 0 0-16.536-1.84M7.5 14.25 5.106 5.272M6 20.25a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0Zm12.75 0a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0Z" />
                            </svg>

                            <p>Cart</p>
                        </button>
                    </div>
                </div>
            </header>
            <nav className='w-full flex items-center justify-center py-3'>
                <ul className='flex items-center justify-between gap-5'>
                    <li>
                        <Link>Best Seller</Link>
                    </li>
                    <li>
                        <Link>Featured Collections</Link>
                    </li>
                    <li>
                        <Link>Shop By Category</Link>
                    </li>
                    <li>
                        <Link>Custom Gift</Link>
                    </li>
                    <li>
                        <Link>Newstand</Link>
                    </li>
                </ul>
            </nav>

        </>
    )
}

export default Navbar